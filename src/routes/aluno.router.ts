...
router.put('/aluno/alterarsenha/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const mensagem: Mensagem = await new AlunoController().alterarsenha(Number(id), req.body);      
    res.json(mensagem);
  } catch (e) {
    next(e);
  }
...
router.get('/aluno', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    alunos.forEach(a => delete a.senha);
    res.json(alunos);
  } catch (e) {
    next(e);
  }
