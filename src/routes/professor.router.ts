...
router.put('/professor/alterarsenha/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const mensagem: Mensagem = await new ProfessorController().alterarsenha(Number(id), req.body);      
    res.json(mensagem);
  } catch (e) {
    next(e);
  }
